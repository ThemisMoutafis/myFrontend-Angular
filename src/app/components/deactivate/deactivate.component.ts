...
export class DeactivateComponent {
  ...
  deactivateAccount() {
    if (this.userService.user()?.role == 'ADMIN') {
      window.alert(
        'This is an admin account. You cannot deactivate an admin account from here.',
      );
      return;
    }
    ...
    if (confirmation) {
      this.userService
        .deactivateUser(this.userService.user()?.username || '')
        .subscribe({
          next: () => {
            console.log(
              `User with username ${this.userService.user()?.username} deactivated succesfully`,
            );
            this.userService.logoutUser();
          },
          error: (error) => {
            console.error(
              `Error activating user with ${this.userService.user()?.username}`,
              error,
            );
          },
        }),
