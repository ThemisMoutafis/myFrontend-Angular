...
export class UserUpdateComponent {
  ...
  form = new FormGroup(
    ...
    ngOnInit() { // Access the signal's current value
      if (this.user) {
  ...
  checkOnSubmit() {
    ...
    if (this.user) {
      ...
      if (this.user()?.username) {
        ...
        this.userService.updateUser(this.user()?.username || " ", userUpdate).subscribe({
          next: (response) => {
            console.log('No Errors:', response);
            const usernameMessage = this.user()?.username || 'user'; 
            this.updateStatus = { success: true, message: `${usernameMessage} successfully updated` };
