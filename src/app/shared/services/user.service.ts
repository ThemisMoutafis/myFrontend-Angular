...
import {
  ...
  PaginatedResponse,
  PaginatedResult,
} from '../interfaces/spring-backend';
...
export class UserService {
  ...
  registerUser(user: User) {
    return this.http
      .post<{ msg: string }>(`${API_URL}/api/users/save`, user)
      .pipe(
  ...
  updateUser(username: String, user: UpdateUser) {
    return this.http
      .put<{ token: string }>(`${API_URL}/api/user/${username}/update`, user)
      .pipe(
        catchError((error: HttpErrorResponse) => {
          const errorResponse: ValidationErrorResponse = error.error;
          return throwError(() => errorResponse);
  ...
  getAllUsersPaginated(
    page: number = 0,
    size: number = 5,
  ): Observable<PaginatedResult<ReadUser>> {
    return this.http
      .get<
        PaginatedResponse<ReadUser>
      >(`${API_URL}/api/users?page=${page}&size=${size}`)
      .pipe(
        map((response) => ({
  ...
  loginUser(credentials: Credentials) {
    return this.http
      .post<{ token: string }>(`${API_URL}/api/auth/login`, credentials)
      .pipe(
        catchError((error: HttpErrorResponse) => {
          const errorResponse: ValidationErrorResponse = error.error;
          return throwError(() => errorResponse);
  ...
  activateUser(username: string): Observable<void> {
    return this.http
      .put<void>(`${API_URL}/api/user/${username}/activate`, {})
      .pipe(
        tap(() => {
  ...
  deactivateUser(username: string): Observable<void> {
    return this.http
      .put<void>(`${API_URL}/api/user/${username}/deactivate`, {})
      .pipe(
        tap(() => {
